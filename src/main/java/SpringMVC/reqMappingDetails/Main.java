package SpringMVC.reqMappingDetails;

/**
 * RequestMapping详解
 * 前面我们已将了解了如何创建一个控制器来处理我们的请求 接着我们只需要在控制器添加一个方法用于处理对应的请求即可 之前我们需要完整地编写一个Servlet来实现
 * 而现在我们只需要添加一个@RequestMapping即可实现 其实从它的名字我们也能得知 此注解就是将请求和处理的方法建立一个映射关系
 ...
 * 我们可以直接在IDEA下方的端点板块中查看当前Web应用程序定义的所有请求映射 并且可以通过IDEA为我们提供的内置Web客户端直接访问某个路径
 *
 * 路径还支持使用通配符进行匹配:
 *
 *      > ? 表示任意一个字符 比如@RequestMapping("/index/x?") 可以匹配/index/xa, /index/xb等
 ...
 *                  public ModelAndView() {
 *                      return new ModeAndView("index");
 *                  }
 *
 * 现在我们如果直接使用浏览器访问此页面 会显示405方法不支持 因为浏览器默认是直接使用GET方法获取页面
 * 而我们这里指定为POST方法访问此地址 所以访问失败 我们现在再去端点中用POST方式去访问 成功得到页面
 *
 *      https://smms.app/image/JVwN2MhrWBAGni9
 *
 ...
 *                  public ModeAndView index() {
 *                      return new ModelAndView("index");
 *                  }
 *
 * 这样 请求参数username不允许为test 并且password必须123 否则无法访问
 *
 * header属性用法与params一致 但是它要求请求头中需要携带什么内容 比如:
 *
 ...
 *
 *      > consumes: 指定处理请求的提交内容类型(Content-Type) 例如application/json, test/html
