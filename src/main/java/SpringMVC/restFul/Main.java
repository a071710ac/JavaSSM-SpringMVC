package SpringMVC.restFul;

/**
 * RestFul风格
 * 中文释义"表现层状态转换" (名字挺高大上的) 它不是一种标准 而是一种设计风格 它的主要作用是充分并正确利用HTTP协议的特征
 * 规范资源获取的URL路径 通俗的讲 RestFul风格的设计允许将参数通过RUL拼接传到服务端 目的是让URL看起来更简洁实用
 * 并且我们可以充分实用多种HTTP请求方法(POST, GET, PUT, DELETE)来执行相同请求地址的不同类型操作
 *
 * 因此 这种风格的连接 我们就可以直接从请求路径中读取参数 比如:
 *
 *                  http://localhost:8080/mvc/index/123456
 *
 * 我们可直接将index的下一级路径作为请求参数进行处理 也就是说现在的请求参数包含在了请求路径中:
 *
